...
# - For Delta: make sure to load openblas and set the LD_LIBRARY_PATH to include the path to libopenblas.so:
#   ml openblas cmake
#   export LD_LIBRARY_PATH=/opt/rh/gcc-toolset-13/root/lib64:/sw/rh9.4/spack/v1.0.0/sw/linux-x86_64_v2/openblas-0.3.29-inh5l3i/lib:$LD_LIBRARY_PATH
...
#For Eigen3:
cd $OMCDIR/eigen
mkdir build_$CPU_FAMILY
cd build_$CPU_FAMILY
CC=mpicc FC=mpifort CXX=mpicxx cmake .. -DCMAKE_BUILD_TYPE=Release -DCMAKE_C_FLAGS_RELEASE="$BUILD_FLAGS" -DCMAKE_Fortran_FLAGS_RELEASE="$BUILD_FLAGS" -DCMAKE_CXX_FLAGS_RELEASE="$BUILD_FLAGS" -DCMAKE_POSITION_INDEPENDENT_CODE=ON -DCMAKE_INSTALL_PREFIX=$OMCDIR/eigen/release_$CPU_FAMILY
...
mkdir build_$CPU_FAMILY
cd build_$CPU_FAMILY
CC=mpicc CXX=mpicxx cmake .. -DCMAKE_BUILD_TYPE=Release -DCMAKE_C_FLAGS_RELEASE="$BUILD_FLAGS" -DCMAKE_CXX_FLAGS_RELEASE="$BUILD_FLAGS" -DCMAKE_POSITION_INDEPENDENT_CODE=ON -DNETCDF_ENABLE_HDF5=ON -DHDF5_ROOT=$OMCDIR/hdf5/release_$CPU_FAMILY -DCMAKE_INSTALL_PREFIX=$OMCDIR/netcdf-c/release_$CPU_FAMILY
VERBOSE=1 make install
...
mkdir build_$CPU_FAMILY
cd build_$CPU_FAMILY
CC=mpicc FC=mpifort CXX=mpicxx cmake .. -DCMAKE_BUILD_TYPE=Release -DCMAKE_C_FLAGS_RELEASE="$BUILD_FLAGS" -DCMAKE_Fortran_FLAGS_RELEASE="$BUILD_FLAGS" -DCMAKE_CXX_FLAGS_RELEASE="$BUILD_FLAGS" -DCMAKE_POSITION_INDEPENDENT_CODE=ON -DENABLE_MPI=ON -DENABLE_HDF5=ON -DHDF5_ROOT=$OMCDIR/hdf5/release_$CPU_FAMILY -DENABLE_NETCDF=ON -DNETCDF_ROOT=$OMCDIR/netcdf-c/release_$CPU_FAMILY -DEIGEN3_DIR=$OMCDIR/eigen/release_$CPU_FAMILY/include/eigen3 -DCMAKE_INSTALL_PREFIX=$OMCDIR/moab/release_$CPU_FAMILY
VERBOSE=1 make install
...
cd $OMCDIR/petsc
make allclean
./configure --configModules=PETSc.Configure --optionsModule=config.compilerOptions --with-cc=mpicc --with-fc=mpifort --with-cxx=mpicxx --with-debugging=0 --prefix=$OMCDIR/petsc/release_$BUILD_NAME COPTFLAGS="$BUILD_FLAGS" CXXOPTFLAGS="$BUILD_FLAGS" FOPTFLAGS="$BUILD_FLAGS" PETSC_ARCH=$BUILD_NAME
